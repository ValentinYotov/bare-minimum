...
# Load .env from the project root regardless of where uvicorn is launched from.
_HERE = os.path.dirname(os.path.abspath(__file__))
load_dotenv(dotenv_path=os.path.join(_HERE, "..", ".env"))

OPENAI_API_KEY     = os.getenv("OPENAI_API_KEY")
CHROMA_PERSIST_DIR = os.path.join(_HERE, "chroma_db")
COLLECTION_NAME    = "crop_profiles_v3"
CSV_PATH           = os.path.join(_HERE, "..", "Crop_recommendation.csv")
RESULTS_FILE       = os.path.join(_HERE, "recommendations_result.txt")
TOP_K              = 15
OPENAI_MODEL       = "gpt-4o-mini"
FEATURE_COLS       = ["N", "P", "K", "ph", "humidity"]

# ----------- FIX: configurable host/port so curl and Dart agree -----------
API_HOST = os.getenv("API_HOST", "0.0.0.0")
API_PORT = int(os.getenv("API_PORT", "8000"))

# Normalisation stats populated at startup from CSV
...
def _make_embedding(feature_values: list[float]) -> list[float]:
    """Normalise feature values to [0, 1] using dataset min/max for cosine similarity."""
...
def query_similar_profiles(
    ...
    query_embedding = _make_embedding(
        [reading.N, reading.P, reading.K, reading.ph, reading.humidity]
    )
...
def get_llm_recommendation(reading: SensorReading, profiles: list[dict]) -> dict:
    ...
    # Retry if fewer than 3 crops were returned
    if len(result.get("recommendations", [])) < 3:
        ...
        messages.append({
            ...
            "content": (
                ...
                "Add more crops using your general agronomic knowledge if the dataset "
                "is not enough. Return the full updated JSON now."
            ),
        })
        result = json.loads(
            client.chat.completions.create(
...
async def lifespan(app: FastAPI):
    ...
    print(f"ChromaDB ready — {collection.count()} profiles loaded")
    yield
...
# ----------- FIX: health-check endpoint so you can verify the server is up -----------
@app.get("/health")
async def health():
    return {
        "status": "ok",
        "collection_loaded": collection is not None,
        "profiles": collection.count() if collection else 0,
    }


@app.post("/recommendations", response_model=RecommendationResponse)
async def recommend(reading: SensorReading):
    # ----------- FIX: guard against collection not being initialised -----------
    if collection is None:
        raise HTTPException(
            status_code=503,
            detail="ChromaDB collection not yet loaded. Try again in a moment.",
        )
...
# ----------- FIX: runnable entry point so `python get_recommendations.py` works -----------
if __name__ == "__main__":
    uvicorn.run(
        "get_recommendations:app",
        host=API_HOST,
        port=API_PORT,
        reload=True,
    )